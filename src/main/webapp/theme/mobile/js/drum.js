...
(function($) {
	...
	var PanelModel = function (index, data_index, settings) 
	{
		...
		this.init = function () {
			...
			$(this.elem).css(
				...
				settings.rotateFn + '(' + -this.angle + 'deg) translateZ(' + 100 + 'px)'
			);
			this.setText();
		};
	...
	var Drum = function(element, options, transformProp)
	{
		...
		var transform = function() {
			$(drum).css(settings.transformProp, 'translateZ(-' + 50 + 'px) ' + settings.rotateFn + '(' + settings.rotation + 'deg)');
			...
			if (selected) {
				...
				$(selected.elem).css({"opacity": "1", "color": "#45bdef"});
				// $(selected.elem).css("opacity", 1);
				$("figure:not(.a" + (selected.angle*100) + ", .hidden)", drum).css({"opacity": "0.9", "color": "#888"});
